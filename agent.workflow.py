...
#langchain的Hub中获取prompt模板，可以进行修改
...
replanner_prompt=ChatPromptTemplate.from_template(
    ...
    你目前已经完成的步骤是：
    {past_steps}
    相应地更新你的计划。如果不需要更多步骤并且可以返回给用户，那么就这样回应。如果需要，填写计划。只添加仍然需要完成的步骤，不要返回语句完成的步骤作为计划的一部分"""
)
...
def should_end(state:PlanExecute)->Literal["agent","__end__"]:
        if "response" in state and state["response"]:
            ...
            return "agent"
from langgraph import START,StateGraph
...
#生成的图片保存到文件
graph_png=app.get_graph().draw__mermaid_png()
with open("agent_workflow.png") as f:
    f.write(graph_png)
#设置配置，递归限制50
